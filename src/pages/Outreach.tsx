...
export default function Outreach() {
  ...
  // Only validate keys that are actually used by the active template.
  const pricingErrors = useMemo(() => {
    ...
    // Quote date — required, must parse to a real date, not in the past.
    if (need("quote_date")) {
    ...
    // Validity — required, non-empty, sensible length.
    if (need("validity")) {
      const v = val("validity");
      if (!v) errs.validity = "Validity is required (e.g. \"7 days\" or until a date).";
      else if (v.length > 80) errs.validity = "Keep validity under 80 characters.";
    }

    // Weekly volume — if present, must parse to > 0 litres.
    if (need("volume")) {
      const v = val("volume");
    ...
    // Fuel prices — only validate the products selected in the mix that are
    // also part of the template. Both ex- and inc-GST must be present and
    // numeric; inc-GST must be at least the ex-GST value.
    (["diesel", "ulp", "adblue"] as const).forEach(p => {
      const exKey = `${p}_price`;
      const incKey = `${p}_price_inc`;
      const used = need(exKey) || need(incKey);
      if (!used) return;
      // If the product isn't in the mix, fields are intentionally blank — skip.
      if (!productMix[p]) return;
      ...
      if (need(exKey)) {
        ...
        else if (n > 10) errs[exKey] = "Looks too high — enter $/L (e.g. 1.8500).";
      }
      if (need(incKey)) {
