    ...
    <div className="text-[#ECE4D2] pb-28 lg:pb-8">
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          <Dialog open={importOpen} onOpenChange={setImportOpen}>
            ...
            <DialogContent className="bg-[#142A16] border-[#2A4A2E] text-[#ECE4D2] max-w-2xl">
              ...
              <p className="text-sm text-[#C7BFAC]">
                Paste a CSV with columns <code className="text-[#ECE4D2]">name, email, org</code>. Each row will be
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[380px_1fr] gap-6">
        ...
        <div className={`rounded-lg border border-[#2A4A2E] bg-[#142A16] p-3 space-y-3 ${selected ? "hidden lg:block" : ""}`}>
          ...
          {loading && (
            <div className="flex items-center gap-2 text-sm text-[#C7BFAC] py-2">
          ...
          <div className="max-h-[calc(100vh-220px)] lg:max-h-[65vh] overflow-y-auto divide-y divide-[#2A4A2E]/50">
            ...
            {!loading && people.length === 0 && (
              <div className="text-sm text-[#C7BFAC] py-6 text-center">No contacts found.</div>
            )}
          </div>
        </div>
            ...
            <>
              <div className="flex items-start justify-between gap-3 flex-wrap">
                <div>
                  ...
                  <div className="text-sm text-[#C7BFAC]">
                ...
                {selectedPipedriveUrl && (
                  <a href={selectedPipedriveUrl} target="_blank" rel="noreferrer"
                     className="text-xs inline-flex items-center gap-1 text-[#C7BFAC] hover:text-[#ECE4D2]">
              ...
              {allVarKeys.some(k => PRICING_KEYS.has(k)) && (
                <div className="space-y-3 rounded-lg border border-[#2A4A2E] bg-[#1f150b] p-3">
                  ...
                  {/* Pricing presets */}
                  <div className="space-y-2 rounded border border-[#2A4A2E] bg-[#0E1F10] p-3">
                    <div className="flex items-center justify-between">
                      <span className="text-[11px] uppercase tracking-wide text-[#C7BFAC]">Presets</span>
                      <span className="text-[10px] text-[#8B8773]">Save the daily Diesel price for quick reuse</span>
                    </div>
                    ...
                    {presets.length === 0 ? (
                      <div className="text-[10px] text-[#8B8773]">No saved presets yet.</div>
                    ) : (
                        ...
                        {presets.map(p => (
                          ...
                          >
                            ...
                            <button
                              ...
                              title="Delete preset"
                              className="text-[10px] text-[#8B8773] hover:text-[#C8F26A] px-1"
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                    {PRICING_META_KEYS.filter(k => allVarKeys.includes(k)).map(key => (
                      <div key={key} className="space-y-1">
                        <span className="text-[11px] text-[#C7BFAC]">
                    ...
                    {ACTIVE_FUELS.map(fuel => {
                      ...
                      return (
                        <div key={fuel} className="grid grid-cols-[80px_1fr_1fr_auto] items-end gap-2">
                          <span className="text-xs text-[#ECE4D2] pb-2">{label}</span>
                          <div className="space-y-1">
                            <span className="text-[10px] text-[#C7BFAC]">Ex-GST $/L</span>
                            <Input
                          ...
                          <div className="space-y-1">
                            <span className="text-[10px] text-[#C7BFAC]">Inc-GST $/L</span>
                            <Input
                              type="number" step="0.0001" inputMode="decimal"
                              value={vars[incKey] ?? ""}
                  ...
                  {allVarKeys.includes("extra_terms") && (
                    <div className="space-y-1">
                      <span className="text-[11px] text-[#C7BFAC]">Extra terms (optional)</span>
                      <Textarea
                        ...
                        className="bg-[#0E1F10] border-[#2A4A2E] text-[#ECE4D2] min-h-[60px]"
                      />
                    </div>
                  )}
                </div>
              ...
              {allVarKeys.filter(k => !PRICING_KEYS.has(k)).length > 0 && (
                <div className="space-y-2">
                  <label className="text-xs uppercase tracking-wide text-[#C7BFAC]">Variables</label>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                    {allVarKeys.filter(k => !PRICING_KEYS.has(k)).map(key => (
                      <div key={key} className="space-y-1">
                        <span className="text-[11px] text-[#C7BFAC] font-mono">{`{{${key}}}`}</span>
                        <Input
                          value={vars[key] ?? ""}
                          onChange={(e) => setVars(v => ({ ...v, [key]: e.target.value }))}
                          placeholder={activeTemplate?.default_values?.[key] ?? ""}
              ...
              <details className="lg:hidden group rounded border border-[#2A4A2E] bg-[#1f150b]">
                ...
                <div className="p-2 border-t border-[#2A4A2E]">
                  <div className="rounded border border-[#2A4A2E] overflow-hidden bg-white">
                    <iframe title="Email preview" srcDoc={safePreviewHtml} className="w-full h-[420px] border-0" />
                  </div>
                  {bcc && (
                    <div className="text-[11px] text-[#C7BFAC] mt-2">
                      BCC: <span className="text-[#ECE4D2]">{bcc}</span>
                    </div>
                  )}
                </div>
              </details>
        ...
        {!editingId ? (
          <div className="space-y-3">
            ...
            <div className="divide-y divide-[#2A4A2E]/50">
              ...
              {templates.length === 0 && (
                <div className="text-sm text-[#C7BFAC] py-6 text-center">No templates yet.</div>
              )}
            </div>
          </div>
        ) : (
