...
import { transactions, driverList, filterByDateRange } from "@/data/mockData";
...
export default function Drivers() {
  ...
  const comparisonData = useMemo(() => {
    return driverList.map((d) => ({
      name: d.name,
      litres: filtered.filter((t) => t.nombre_vendedor === d.name).reduce((s, t) => s + t.cantidad, 0),
    }));
  }, [filtered]);
    ...
    <div className="space-y-6 max-w-5xl">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        {driverList.map((driver, i) => {
          const driverTxns = filtered.filter((t) => t.nombre_vendedor === driver.name);
          const totalLitres = driverTxns.reduce((s, t) => s + t.cantidad, 0);
          const avgSize = driverTxns.length > 0 ? Math.round(totalLitres / driverTxns.length) : 0;

          // Trucks used
          const trucksUsed = [...new Set(driverTxns.map((t) => t.estacion))];

          const dailyMap: Record<string, number> = {};
          driverTxns.forEach((t) => { dailyMap[t.date] = (dailyMap[t.date] || 0) + t.cantidad; });
          const dailyData = Object.entries(dailyMap)
            .sort(([a], [b]) => a.localeCompare(b))
            .map(([date, litres]) => ({ date: format(parseISO(date), "dd MMM"), litres }));

          return (
            <div key={driver.name} className="glass-card p-4 sm:p-5 animate-fade-in" style={{ animationDelay: `${i * 50}ms` }}>
              <div className="mb-4">
