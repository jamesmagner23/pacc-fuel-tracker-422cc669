...
function useDeleteUser() {
  ...
  return useMutation({
    mutationFn: async (userId: string) => {
      // Delete user_roles entry (we can't delete auth user from client side, just the role)
    ...
    <div className="flex flex-col gap-5 max-w-[1100px]">
                ...
                {users.map((user) => (
                  <tr key={user.id} className="border-b border-border/50 hover:bg-muted/30 transition-colors">
                    ...
                    <td className="py-2.5 pr-3">
                      <span className={`text-[10px] font-semibold uppercase px-2 py-0.5 rounded-full ${ROLE_COLORS[user.role] || ""}`}>
                        {user.role}
                      </span>
                    </td>
                    ...
                    <td className="py-2.5">
                      <div className="flex gap-1">
                        <button
                          ...
                          className="bg-transparent border-none cursor-pointer text-muted-foreground hover:text-foreground p-1 transition-colors"
                          title="Edit"
                        >
                        ...
                        {user.role !== "admin" && (
                          <button
                            onClick={() => {
                              if (confirm(`Remove ${user.full_name || user.email}?`)) {
                                deleteUser.mutate(user.user_id);
                              }
                            }}
                            className="bg-transparent border-none cursor-pointer text-muted-foreground hover:text-destructive p-1 transition-colors"
                            title="Delete"
                          >
      ...
      <div className="bg-surface border border-surface-border rounded-[10px] p-4 sm:p-5">
        ...
        {activityLoading ? (
          ...
          <div className="text-muted-foreground text-[13px]">No activity logged yet. Activity will appear as users log in and use the portals.</div>
        ) : (
          ...
          <div className="bg-surface border border-surface-border rounded-[10px] p-5 w-full max-w-[400px]" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex flex-col gap-3">
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <div className={`text-[12px] font-semibold uppercase px-2 py-1 rounded-full w-fit ${ROLE_COLORS[editingUser.role]}`}>
                  {editingUser.role}
                </div>
              </div>
              <button
                onClick={handleSaveEdit}
                className="bg-primary text-primary-foreground border-none rounded-full px-5 py-2 text-xs font-semibold cursor-pointer mt-2"
              >
                Save Changes
              </button>
            </div>
